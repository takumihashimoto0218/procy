...
export default function DashboardPage() {
  ...
  const getStatusBadge = (status: ApplicationStatus) => {
    const statusConfig = {
      ...
      applied: { label: "出願済み", className: "bg-yellow-100 text-yellow-800" },
  ...
  const fetchUniversityData = async () => {
    try {
      ...
      // データを型安全に変換
      const formattedData: UniversityWithStatus[] = (data as SupabaseResponse[]).map((item) => ({
        id: item.id,
        ...
        application_period_start: item.departments?.application_period_start ?? null,
        application_period_end: item.departments?.application_period_end ?? null,
  ...
  const adjustToAcademicYear = (dateStr: string | null, currentYear: number): string | null => {
    ...
    try {
      ...
      const academicYear = new Date().getMonth() + 1 >= 4 ? currentYear : currentYear - 1;
      const year = monthNum >= 1 && monthNum <= 3 ? academicYear + 1 : academicYear;
  ...
  const getCellBackground = (uni: UniversityWithStatus, date: Date): string => {
    if (!uni.application_period_start || !uni.application_period_end || !uni.exam_date || !uni.result_date) {
      return "";
    }
    ...
    try {
      ...
      const startDateStr = adjustToAcademicYear(uni.application_period_start, currentYear);
      const endDateStr = adjustToAcademicYear(uni.application_period_end, currentYear);
  ...
  const getDates = () => {
    ...
    const year = currentMonth >= 4 ? today.getFullYear() : today.getFullYear() - 1;
    ...
    <div className="min-h-screen bg-white">
        ...
        <div className="px-8 py-3">
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-blue-500 font-medium">AdmissionPlanner</span>
            </div>
      ...
      <div className="px-8 py-4">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-xl font-medium text-black">大学受験スケジュール管理</h1>
          <div className="w-[200px]"></div>
        </div>

        <div className="flex gap-6">
          <div className="w-[350px] bg-white rounded border border-gray-200 p-4">
            <h2 className="font-medium text-sm mb-4 text-black">大学 / 受験方式</h2>
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-grow">
                      <div className="flex items-center justify-between">
                        <span className="font-medium text-black">{uni.schoolName}</span>
                        <button
                      ...
                      <div className="mt-2">
                        <span className={`inline-block px-2 py-0.5 rounded text-xs ${getStatusBadge(uni.status).className}`}>
          ...
          <div className="flex-1 border border-gray-200 rounded bg-white p-6 overflow-x-auto">
            <div className="min-w-[1200px]">
              <div className="grid" style={{ gridTemplateColumns: `repeat(${dates.length}, minmax(40px, 1fr))` }}>
                ...
                {universities.map((uni) =>
                  dates.map((date, dateIndex) => (
                    <div
                      key={`cell-${uni.userUniversityId}-${date.toISOString().split("T")[0]}-${dateIndex}`}
                      className={`h-24 border-b border-r border-gray-100 ${getCellBackground(uni, date)}`}
                    />
                  ))
                )}
              </div>
