  ...
  const fetchUniversityData = async () => {
    try {
      ...
      if (!data) return;
			...
			const formattedData: UniversityWithStatus[] = data.map((item: RawUniversityData) => ({
				id: item.schools[0].id,           // 配列の最初の要素を使用
				schoolName: item.schools[0].name,
				facultyName: item.faculties[0].name,
				departmentName: item.departments[0].name,
				application_period_start: item.departments[0].application_period_start,
				application_period_end: item.departments[0].application_period_end,
				exam_date: item.departments[0].exam_date,
				result_date: item.departments[0].result_date,
				status: item.status,
